...
#         # A simple XOR operation as the round function
#         return text ^ key

#     for _ in range(rounds):
#         left, right = text >> 32, text & 0xFFFFFFFF
#         new_right = round_function(right, key)
#         new_right = new_right ^ left
#         text = (right << 32) | new_right

#     return text

# def encrypt(plaintext, key, rounds):
#     plaintext_bytes = plaintext.encode('latin1')  # Convert text to bytes using 'latin1' encoding
#     plaintext_bytes += b'\0' * (8 - len(plaintext_bytes) % 8)  # Pad the bytes if needed
#     encrypted = b''

#     for i in range(0, len(plaintext_bytes), 8):
#         block = int.from_bytes(plaintext_bytes[i:i+8], 'big')
#         encrypted_block = feistel_cipher(block, rounds, key)
#         encrypted += encrypted_block.to_bytes(8, 'big')
...
#         decrypted_block = feistel_cipher(block, rounds, key)
#         decrypted += decrypted_block.to_bytes(8, 'big')

#     return decrypted.rstrip(b'\0').decode('latin1')  # Remove padding and decode
...
# print("Cipher Text:", encrypted_text.decode('latin1'))

# # Decryption
# decrypted_text = decrypt(encrypted_text, key, rounds)
# print("Retrieved Plain Text is:", decrypted_text)
